# models/networks/mambavision_small_progressive_region_sod.py
...
class HierarchicalRegionDecomposition(nn.Module):
    """
    Decompose RGB and hierarchical region-mean maps into:
    ...
    The RGB image is already ImageNet-normalized by the dataset.
    Region-mean maps are stored in [0, 1], so they are normalized
    into the same space before decomposition.
    """
...
class RegionScaleEncoder(nn.Module):
    """
    Encode one hierarchical region component at the spatial
    resolution and channel dimension of a backbone stage.
    """
...
class RegionPyramidEncoder(nn.Module):
    """
    Map the region hierarchy onto the four MambaVision stages.
...
class BidirectionalRegionVisualInteraction(nn.Module):
    """
    Bidirectional cross-attention between a visual feature and
    its corresponding region-hierarchy feature.

    Visual queries attend to region features:

        Visual <- Region

    Region queries also attend to visual features:

        Region <- Visual

    Both results are fused back into the visual feature.
    """
    ...
    def forward(
        ...
        visual_normalized = self.visual_norm(
            visual_feature
        )

        region_normalized = self.region_norm(
            region_feature
        )
...
class DepthwiseDilatedBranch(nn.Module):
    """
    Cheap depthwise visual branch used by the Stage2
...
class RegionConditionedLocalReconstruction(nn.Module):
    """
    Stage2 local reconstruction.

    The region feature does not simply gate the visual feature.
    Instead, it predicts a spatial mixture over three dilated
    depthwise visual operators.

        dilation = 1
        dilation = 2
        dilation = 3

    It also predicts channel-wise affine modulation.
    """
    ...
    def forward(
        ...
        for index, branch_feature in enumerate(
            branch_outputs
        ):
            mixed_feature = (
                mixed_feature
...
class LocalDetailReconstruction(nn.Module):
    ...
    Full attention is avoided at 88 x 88. Visual and region-detail
    features interact through reduced-channel local representations.
    """
...
class GlobalSemanticBranch(nn.Module):
    """
    Preserve the region-conditioned highest-level semantic feature
    and inject it directly into each decoder stage.
    """
...
class MambaVisionSmallProgressiveRegionSOD(
    nn.Module
):
    """
    MambaVision-S progressive SOD with hierarchical region
    reconstruction.
    ...
    Backbone reconstruction:
        ...
        Stage2 + R2:
            region-conditioned multi-dilation local reconstruction
    ...
    The reconstructed Stage4 is then used by both the deep decoder
    entry and all persistent global semantic branches.
    """
    ...
    def __init__(
        ...
        # -------------------------------------------------
        # Persistent Stage4 semantic stream
        # -------------------------------------------------
    ...
    def forward(
        ...
        # -------------------------------------------------
        # Backbone feature reconstruction
        # -------------------------------------------------
...
def build_model(
) -> MambaVisionSmallProgressiveRegionSOD:
    return MambaVisionSmallProgressiveRegionSOD(
        pretrained_path=PRETRAINED_PATH,
    )