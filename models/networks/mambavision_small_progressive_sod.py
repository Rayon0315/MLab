# models/networks/mambavision_small_progressive_sod.py
...
class GlobalSemanticBranch(nn.Module):
    """
    Preserve the highest-level semantic information and inject it
    directly into different decoder stages.

    Stage4 is globally pooled first, then projected to the target
    channel dimension and expanded to the target spatial size.
    """
...
class ProgressiveSelectiveFusion(nn.Module):
    ...
    Inputs:
        ...
        global_feature:
            direct semantic context from Stage4
...
class MambaVisionSmallProgressiveSOD(nn.Module):
    """
    MambaVision-S with a progressive hierarchical decoder.
    ...
    Stage4 also provides persistent global semantic context
    directly to all three fusion stages.
    """
    ...
    def __init__(
        ...
        # Deep decoder entry
        #
        # Keep much more of Stage4 capacity than the
        # original 768 -> 128 projection.
        # -------------------------------------------------
        ...
        # Preserve the baseline boundary refinement.
    ...
    def forward(
        ...
        # -------------------------------------------------
        # Keep the original baseline boundary refinement.
        # -------------------------------------------------
...
def build_model() -> MambaVisionSmallProgressiveSOD:
    return MambaVisionSmallProgressiveSOD(
        pretrained_path=PRETRAINED_PATH,
    )