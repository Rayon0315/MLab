...
class VMambaSmallFPNNAMLabPyramidAsymFGV2SOD(
    VMambaSmallFPNNAMLabHybridSOD
):
    """
    Asymmetric FG-only v2.

    Compared with v1:
        - same foreground-only prototype topology;
        - explicit fg_support_logits;
        - direct semantic support supervision through the training wrapper;
        - learnable residual_scale starting at zero.

    This isolates whether semantically anchoring FG support fixes the v1
    failure where hard BG received even higher support than confident GT-FG.
    """
...
def build_model(
    ...
    return VMambaSmallFPNNAMLabPyramidAsymFGV2SOD(
        pretrained_path=PRETRAINED_PATH,
