# models/networks/mambavision_small_cfm_upernet_sod.py
...
from models.components.segmentation_heads import UPerNetHead
...
class MambaVisionSmallCFMUPerNetSOD(nn.Module):
    """
    RGB-only MambaVision-S + CFM feature modeling + UPerNet head.

    This network is intentionally identical to the SegFormer variant up to
    the four CFM-enhanced backbone features. Only the prediction head changes.
...
def build_model() -> MambaVisionSmallCFMUPerNetSOD:
    return MambaVisionSmallCFMUPerNetSOD(
        pretrained_path=PRETRAINED_PATH,
