...
class VMambaSmallFPNNAMLabPyramidAsymEvidenceV2AuxOnlySOD(
    VMambaSmallFPNNAMLabHybridSOD
):
    """
    Asymmetric Evidence v2 + BG AuxOnly.

    This is a causal ablation of the full v2 model.

    FG path:
        unchanged from Asymmetric Evidence v2
        - foreground-only prototype
        - semantically supervised fg_support_logits
        - learnable residual_scale

    BG path:
        unchanged verifier inputs / FP-risk supervision
        - detached raw Stage2 / Stage3
        - detached coarse probability
        - detached FG support
        - mismatch = p * (1 - fg_support)

    Critical difference:
        main prediction = coarse_logits

    The verifier still computes a hypothetical suppressed prediction for
    diagnostics, but the main SOD loss no longer sees:
        coarse_logits - suppression

    Therefore:
        L_main -> coarse_logits
        L_fg   -> FG support branch
        L_fp   -> BG verifier

    This directly tests whether the v2 degradation came from letting
    subtraction alter the main training objective rather than from the
    evidence modules themselves.
    """
    ...
    def forward(
        ...
        # Diagnostic-only branch. This value is NOT used as outputs["pred"].
        suppressed_logits = bg[
            "refined_logits"
        ]
        ...
        return {
            # Main SOD supervision sees the unmodified coarse prediction.
            "pred": upsample(coarse_logits),
            "coarse_pred": upsample(coarse_logits),

            # Hypothetical test-time suppression for diagnosis only.
            "suppressed_pred": upsample(
                suppressed_logits
            ),
...
def build_model(
) -> VMambaSmallFPNNAMLabPyramidAsymEvidenceV2AuxOnlySOD:
    return (
        VMambaSmallFPNNAMLabPyramidAsymEvidenceV2AuxOnlySOD(
            pretrained_path=PRETRAINED_PATH,
